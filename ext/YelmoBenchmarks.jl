...
# -----------------------------------------------------------------------
# CalvingMIP calving-law Yelmo wrappers.
#
# The pure-array math lives in IceSheetBenchmarks core
# (src/calvingmip.jl). These wrappers match the Yelmo
# `YelmoHooks.calv_flt` signature — Oceananigans Field arguments — do
# the halo-fill, take 2D views of the Field interiors (which are
# 3D `(Nx, Ny, 1)` under Flat-z), then delegate to the array core.
#
...
# 2D view of a (Nx, Ny, 1) field interior.
@inline _interior2d(field) = view(interior(field), :, :, 1)

function IceSheetBenchmarks.calvmip_exp1!(cr_x::AbstractField, cr_y::AbstractField,
                                           u_bar::AbstractField, v_bar::AbstractField,
    ...
    IceSheetBenchmarks.calvmip_exp1!(
        _interior2d(cr_x), _interior2d(cr_y),
        _interior2d(u_bar), _interior2d(v_bar),
        H_ice, f_ice, lsf, time;
        xc = xc, yc = yc, r_lim = r_lim)
    return cr_x, cr_y
end

function IceSheetBenchmarks.calvmip_exp2!(cr_x::AbstractField, cr_y::AbstractField,
                                           u_bar::AbstractField, v_bar::AbstractField,
                                           H_ice, f_ice, lsf, time::Real;
                                           xc::AbstractVector{<:Real},
                                           yc::AbstractVector{<:Real})
    fill_halo_regions!(u_bar)
    fill_halo_regions!(v_bar)
    IceSheetBenchmarks.calvmip_exp2!(
        _interior2d(cr_x), _interior2d(cr_y),
        _interior2d(u_bar), _interior2d(v_bar),
        H_ice, f_ice, lsf, time;
        xc = xc, yc = yc)
